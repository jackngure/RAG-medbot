...
def generate_daily_analytics():
    ...
    try:
        ...
        # Calculate metrics - fix query syntax and logic
        analytics.total_users = UserProfile.objects.filter(
            last_seen__date__lte=yesterday
        ).count()
        ...
        analytics.location_shares = EmergencyLog.objects.filter(
            Q(timestamp__date=yesterday) & Q(location_shared=True)
        ).count()
        ...
        # Top diseases - more efficient approach
        ...
        for log in top_diseases:
            if log:  # Check if log is not None
                try:
                    # Handle if matched_diseases is a list/dict
                    diseases = log if isinstance(log, (list, tuple)) else [log]
                    ...
                    print(f"Error processing disease log: {e}")
                    continue
